...
for index in reports:
    reports_string = reports_string + f"{index['appname']}   --/--   {index['appdate']}   --/--   {index['appcomment']}\n"
...
class UI(customtkinter.CTkScrollableFrame):
    def __init__(self, master, **kwargs):
        ...
        self.banner = customtkinter.CTkLabel(self.login_frame, text="All activity loaded; it's possible that some of the reports \nare not saved in the SQL database.",
...
class App(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        self.login_frame = UI(master=self, width=360, height=200)
        self.login_frame.grid(row=0, column=0, padx=20, pady=20)
        self.title("SENTRY - Activity Report")
